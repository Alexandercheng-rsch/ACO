...
# @njit(parallel=True)
def intraswap(ants_paths, demand, capacity, drivers, distances, repair=False, gap=0):
    repaired = False
            ...
            while timeout < 500:
                ...
                if repair:
                    if ((path_demand1 - demand1 + demand2 <= capacity) &
                        ...
                        ant_path[driver1, -1] = path_dist(ant_path[driver1, 0:driverlen1], distances)
                        ant_path[driver2, -1] = path_dist(ant_path[driver2, 0:driverlen2], distances)
                        ant_path[driver1, -2] = path_demand1 - demand1 + demand2
                        ant_path[driver2, -2] = path_demand2 - demand2 + demand1
                        if ant_path[driver1, -2] + gap <= capacity or ant_path[driver2, -2] + gap <= capacity:
                            ants_paths[driver1, :, i, k] = ant_path[driver1, :]
                            ...
                            ants_paths[driver1, -2, i, k] = ant_path[driver1, -2]
...
def gen_paths(drivers, customers, pheromone, alpha, beta, rho, n_ants, demand, capacity, distance, colonies, optimise):
    ...
    if optimise:
        ...
        paths, _ = intraswap(paths, demand, capacity, drivers, distance, False)
    return paths
...
def choose_node(unvisited, pheromones, distance, feasibility, alpha, beta):
    probability = np.where((distance != 0) & (unvisited != 0) & (feasibility != 0),
                           (pheromones ** alpha) * ((1 / distance) ** beta),
                           0)
